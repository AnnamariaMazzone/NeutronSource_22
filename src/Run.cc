...
void Run::EndOfRun() 
{
  ...
  //run condition
  //
    G4Material* material = fDetector->GetContainMaterial();
  G4String Particle = fParticle->GetParticleName();    
  G4cout << "\n The run is " << numberOfEvent << " "<< Particle << " of "
         << G4BestUnit(fEkin,"Energy") << " within " 
         << material->GetName() << " (D =  "
         << G4BestUnit(2*(fDetector->GetContainerRadius()),"Length") << " L = "
         << G4BestUnit(fDetector->GetContainerLength(),"Length") << ")" << G4endl;
